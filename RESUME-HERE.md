...
| Unit | Scope | Status |
|---|---|---|
| P5 | Promote parallelism to a real unit — `std::thread::scope` level-parallel `subset_construction` | **done, SHIPPED (safe backend).** Opus implementer built BOTH a persistent-unsafe-pool and a zero-unsafe scoped backend; quiet-machine A/B (median-of-6, alternating): scoped within noise on 230/286/rsp1 (0.6% faster to 2.4% slower), pool 11.5% faster ONLY on rsp2. **User chose scoped** — the pool's edge on the single heaviest workload not worth the first `unsafe` in a trust-critical crate. Pool + all 11 `unsafe` lines DELETED (`pool.rs`→`parallel.rs`, coordination primitives only); **wr-core is `unsafe`-free again**. Fable+Sonnet split-context review (own worktrees): ZERO correctness/bit-identity defects — concurrency protocol verified static + mutation (removing the shutdown-wake HANGS the panic test) + digest identical across 1–64 threads; two real findings fixed by coordinator: an embedder in-process opt-out (`wr_core::set_thread_count`, so ct-research can force sequential without an env var) and a `u32` truncation guard (ostrowski pattern). Gates all green: workspace 51 suites; golden EXACT `675|670|1(383)|4|0`; diffgen 50,000/0; 8-process determinism digest identical. **OPEN FOLLOW-UP: `DEFAULT_MAX_THREADS=7` is a PROVISIONAL placeholder — the D3 worker-count sweep (2/4/6/max, quiet machine) that sets the real default was deferred because the machine wouldn't quiet; tracked in `docs/BACKLOG-D3-THREAD-TUNING.md`. Tuning knob only, bit-identical at every value.** |
