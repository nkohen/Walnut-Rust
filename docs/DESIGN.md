...
**How we build it.** Mechanical port first (faithful Java→Rust transliteration preserving behavior, à la the [Bun rewrite](https://bun.com/blog/bun-in-rust)), refactored toward idiomatic Rust later — because a faithful port is what makes tight differential testing against the Java original possible. Execution is AI-orchestrated using the adversarial-review loop this repo is already set up for (implementer → two split-context reviewers → fixer), sharded across the existing container-agent fleet.
...
**We adapt directly, and this repo is already tooled for it:**

- **The adversarial loop.** Reuse the review agents already here — `proof-referee`, `claim-auditor`, `red-team` — plus generic implementer/reviewer roles. For *math* code the reviewers' standing instruction is "assume a mathematical mistake exists; find it," and they see only the diff (split context prevents author bias — the Bun lesson).
