...
//! ```
//!
//! Cost: two relaxed **loads** (the "linked" and "enabled" flags) per allocation and one
//! per free while counting is off, and one relaxed atomic add on a shared counter per
//! allocation/free once counting is on (`memory_meter::enable` — which every `Prover`/
//! `Engine` calls at construction when this wrapper is linked, and which a memory cap
//! requires). A direction-only A/B of the two paths is recorded in
//! `docs/CT-RESEARCH-INTEGRATION.md`.
