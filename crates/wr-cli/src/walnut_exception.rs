...
/// Runs `f`, converting a panic escaping it into `Err(panic message)` — Rust's stand-in for
/// Java's `catch (RuntimeException e)` around a `wr-core` guard that ports a
/// `WalnutException` as a `panic!`/`assert!`.
///
/// Introduced here by Phase 3b's U23, for `Prover.dispatch`'s handler; **moved to
/// [`wr_core::walnut_panic`] by U27**, because `wr-logic`'s `eval::compute` needs the same
/// mechanism for `Commands/EvalDef.compute`'s own inner `catch (RuntimeException e)`
/// (`EvalDef.java:123-128`) and `wr-logic` cannot depend on this crate. Re-exported under the
/// original name so every existing call site here is unchanged; see that module's docs for
/// the full rationale, the console-noise handling, and the guard-authoring rule about
/// `assert_eq!` payloads.
pub use wr_core::walnut_panic::catch_walnut_panic;
...
mod tests {
    ...
    // `catch_walnut_panic`'s own three tests
    // (`..._passes_a_success_through_untouched`, `..._recovers_both_panic_payload_shapes`,
    // `..._restores_the_silence_flag_on_both_paths`) MOVED verbatim to
    // `wr_core::walnut_panic`'s test module along with the function itself (U27) — they touch
    // its private `SILENCE_PANIC` thread-local, which cannot be reached through the re-export.
    // Two more (`nested_calls_restore_the_outer_flag_not_a_default`,
    // `a_message_less_payload_is_still_an_error_with_an_empty_message`) were added there at the
    // same time. Nothing was dropped.

    /// Still exercised from this crate: the re-export really is callable under the old name,
    /// so no `wr-cli` call site silently lost its guard in the move.
    #[test]
    fn the_reexported_guard_is_callable_under_its_original_name() {
